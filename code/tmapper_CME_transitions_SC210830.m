...
% SC: we'll use this script to explore transition plots across different
% parameters.
% - Switching between this script and tmapper_CME_task.m often
...
% SC: load the following to get access to x_label
load([ savepath 'sumdat_tmapper_cyc_' par2filename({'k',5,'d',2}) '.mat'])

% SC: this is what we really need
% also need to go back to tmapper_CME_task to compute the sumdat variable
...
N_tasks = 5;
N_t = length(x_label); % number of TRs


%% ===== compute source/sink distance for each subject ===== %%
% SC: If things have been saved, skip over to bottom of next block
...
save([ savepath 'sumdat_tmapper_stats_' par2filename({'k',k,'d',d}) '.mat'],'srdisMean','srdisSE','skdisMean','skdisSE','ssdiffMean','ssdiffSE')

load([ savepath 'sumdat_tmapper_stats_' par2filename({'k',k,'d',d}) '.mat']) %if previously computed
